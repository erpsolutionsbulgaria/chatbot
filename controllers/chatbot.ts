...
export const detectIntent = async (queryInput: any, sessionId: any, queryParams: any): Promise<any> => {
  ...
  const intentName = (result?.intent != null) ? result.intent.displayName : 'null'
  const fulfillmentMessages = result?.fulfillmentMessages
  const contexts = responses[0].queryResult?.outputContexts
...
const textQuery = async (data: any): Promise<any> => {
  ...
  if (!('is_echo' in message)) {
    ...
    const result = await detectIntent(queryObj, messageObj.sender.id, {})
    ...
    switch (result.intentName) {
      ...
      case intentConsts.SPECIAL_OFFERS: await obj.sendOffers(); break
      case intentConsts.WELCOME_INITIAL_FAQ:
      case intentConsts.DEFAULT_WELCOME_WISMO:
        await obj.sendWelcomeIntent(result.fulfillmentMessages); break
